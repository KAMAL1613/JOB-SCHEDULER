...
export default function Login({ onLoginSuccess }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(err.message || "Failed to login. Please check details.");
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="max-w-md w-full space-y-8 bg-white border border-slate-200/80 p-8 sm:p-10 rounded-3xl shadow-xl relative overflow-hidden">
                ...
                <div className="text-center">
                    ...
                    <p className="mt-2 text-sm text-slate-500">
                        Sign in to monitor your distributed queues
                    </p>
                </div>

                {error && (
                    <div className="bg-rose-50 border border-rose-250 text-rose-600 text-sm px-4 py-3 rounded-xl">
                        {error}
                    </div>
                )}
